...
static uacpi_size
fixed_event_counters[UACPI_FIXED_EVENT_MAX + 1];
...
uacpi_u8 gpe_bit(struct gp_event *event)
{
    return event->idx - event->reg->base_idx;
}

uacpi_u8 gpe_mask(struct gp_event *event)
{
    return 1 << gpe_bit(event);
}
...
static uacpi_status clear_gpe(struct gp_event *event)
{
    ...
    return uacpi_gas_write(&reg->status, gpe_bit(event));
}
...
static void do_restore_gpe(uacpi_handle opaque)
{
    uacpi_status ret;
...
static void do_run_gpe_handler(uacpi_handle opaque)
{
    uacpi_status ret;
    struct gp_event *event = opaque;
    ...
    ret = uacpi_kernel_schedule_work(
        UACPI_WORK_NOTIFICATION, do_restore_gpe, event
    );
    if (uacpi_unlikely_error(ret)) {
        uacpi_error("unable to schedule GPE(%02X) restore: %s\n",
                    event->idx, uacpi_status_to_string(ret));
        do_restore_gpe(event);
    }
}
...
)
{
    ...
    case GPE_HANDLER_TYPE_IMPLICIT_NOTIFY:
        ret = uacpi_kernel_schedule_work(
            UACPI_WORK_GPE_EXECUTION, do_run_gpe_handler, event
        );
        if (uacpi_unlikely_error(ret)) {
            uacpi_warn(
...
static void event_release_implicit_notify_handlers(struct gp_event *event)
{
...
static void uninstall_gpe_block(struct gpe_block *block)
{
    ...
    if (block->events != UACPI_NULL) {
        ...
        for (i = 0; i < block->num_events; ++i) {
            ...
            case GPE_HANDLER_TYPE_IMPLICIT_NOTIFY: {
                event_release_implicit_notify_handlers(event);
                break;
            }
...
static struct gp_event *event_from_block(struct gpe_block *block, uacpi_u16 idx)
{
    uacpi_u16 offset;
...
)
{
    ...
    event = event_from_block(block, idx);
    if (event == UACPI_NULL)
        return UACPI_NS_ITERATION_DECISION_CONTINUE;
...
)
{
    ...
    uacpi_namespace_for_each_node_depth_first(
        device_node, do_match_gpe_methods, block
    );

    uacpi_trace("initialized GPE block: %.4s@[%d->%d] (IRQ %d)\n",
...
)
{
    ...
    ctx->out_block = block;
    ctx->out_event = event_from_block(block, ctx->idx);
...
static uacpi_status event_remove_user(struct gp_event *event)
{
    ...
    if (--event->num_users == 0) {
        event->reg->runtime_mask &= ~gpe_mask(event);
...
static uacpi_status event_add_user(
...
static uacpi_bool event_needs_polling(struct gp_event *event)
{
...
)
{
    ...
    for (i = 0; i < block->num_registers; ++i) {
        for (j = 0; j < EVENTS_PER_GPE_REGISTER; ++j) {
            ...
            ret = event_add_user(event, EVENT_CLEAR_IF_FIRST_NO);
            if (uacpi_unlikely_error(ret)) {
            ...
            *poll_blocks |= event_needs_polling(event);
            count_enabled++;
        }
    }
...
static uacpi_status sanitize_device_and_find_event(
    uacpi_namespace_node **gpe_device, uacpi_u16 idx,
    struct gp_event **out_event
)
{
...
)
{
    ...
    ret = sanitize_device_and_find_event(&gpe_device, idx, &event);
    if (uacpi_unlikely_error(ret))
        return ret;
    ...
    event->handler_type = native_handler->previous_triggering;

    if ((event->handler_type == GPE_HANDLER_TYPE_AML_HANDLER ||
         event->handler_type == GPE_HANDLER_TYPE_IMPLICIT_NOTIFY) &&
         native_handler->previously_enabled) {
        event_add_user(event, EVENT_CLEAR_IF_FIRST_NO);

        if (event_needs_polling(event))
...
)
{
    ...
    ret = event_add_user(event, EVENT_CLEAR_IF_FIRST_YES);
    if (uacpi_unlikely_error(ret))
        return ret;
...
)
{
    ...
    return event_remove_user(event);
}
...
{
    return gpe_mask_unmask(gpe_device, idx, UACPI_TRUE);
}
...
{
    return gpe_mask_unmask(gpe_device, idx, UACPI_FALSE);
}
